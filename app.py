...
GOVERNMENT_WARNINGS_FILE = os.path.join(DATA_DIR, 'government_warnings.json')
...
def get_interaction_recommendation(drug1, drug2):
    ...
    recommendations = {
        ...
        ('lisinopril', 'ibuprof-en'): "Monitor blood pressure and kidney function. Use lowest effective dose of NSAID."
    }
...
def check_government_warnings(drug_list):
    """Check for active government warnings for prescribed drugs"""
    warnings = load_json_file(GOVERNMENT_WARNINGS_FILE)
    active_warnings = []
    ...
    for drug_name in drug_list:
        for warning in warnings:
            if (warning['drug_name'].lower() == drug_name.lower() and 
                warning['status'] == 'active' and
                warning['active_from'] <= current_date <= warning['active_until']):
...
def wizard(prescription_id):
    ...
    # Only get basic data, no processing warnings until user clicks process
    inventory_status = []
...
def process_step():
    ...
    if step == 1:  # Prescription Validation
        ...
        if doctor and doctor.get('chamber_verified'):
            ...
            if not doctor:
                return jsonify({'status': 'error', 'message': 'Doctor verification failed', 'details': f'Doctor ID {prescription["doctor_id"]} not found in medical registry'})
            elif doctor.get('status') == 'suspended':
                return jsonify({'status': 'error', 'message': 'Doctor verification failed', 'details': f'Dr. {doctor["name"]} license is currently suspended'})
            elif doctor.get('status') == 'inactive':
                return jsonify({'status': 'error', 'message': 'Doctor verification failed', 'details': f'Dr. {doctor["name"]} license has expired or is inactive'})
            else:
                return jsonify({'status': 'error', 'message': 'Doctor verification failed', 'details': f'Dr. {doctor["name"]} not verified with Medical Chamber'})
        ...
        if patient and patient.get('insurance_status') == 'active':
            ...
            if not patient:
                return jsonify({'status': 'error', 'message': 'Patient identity verification failed', 'details': f'Patient ID {prescription["patient_id"]} not found in registry'})
            elif patient.get('insurance_status') == 'expired':
                return jsonify({'status': 'error', 'message': 'Patient identity verification failed', 'details': f'Patient {patient["name"]} insurance has expired'})
            elif patient.get('insurance_status') == 'suspended':
                return jsonify({'status': 'error', 'message': 'Patient identity verification failed', 'details': f'Patient {patient["name"]} insurance is currently suspended'})
            elif patient.get('insurance_status') == 'pending':
                return jsonify({'status': 'error', 'message': 'Patient identity verification failed', 'details': f'Patient {patient["name"]} insurance verification is pending'})
            else:
                return jsonify({'status': 'error', 'message': 'Patient identity verification failed', 'details': f'Patient {patient["name"]} insurance status is invalid'})
        ...
        if interactions:
            ...
            interaction_details = '; '.join([f'{i["drug1"]} + {i["drug2"]}: {i["warning"]}' for i in interactions[:2]])
            if high_risk:
                return jsonify({'status': 'error', 'message': f'Drug safety check failed', 'details': f'High-risk interactions detected between {interaction_details}. Additional {len(interactions)-2 if len(interactions) > 2 else 0} interactions found.'})
            else:
                return jsonify({'status': 'error', 'message': f'Drug safety check failed', 'details': f'Moderate interactions detected between {interaction_details}. Review required before dispensing.'})
        else:
        ...
        if all_available:
            ...
            return jsonify({'status': 'error', 'message': f'Inventory check failed', 'details': f'Insufficient stock for: {", ".join(low_stock_drugs)}. Contact supplier for emergency ordering.'})
    
    elif step == 7:  # Government Warnings
        drug_list = [drug['name'] for drug in prescription['drugs']]
        government_warnings = check_government_warnings(drug_list)
        if government_warnings:
            critical_warnings = [w for w in government_warnings if w['severity'] == 'critical']
            warning_summary = '; '.join([f'{w["drug_name"]}: {w["title"]}' for w in government_warnings[:2]])
            if critical_warnings:
                return jsonify({'status': 'error', 'message': f'Government warnings check failed', 'details': f'Critical health alerts for {warning_summary}. Additional restrictions may apply.'})
            else:
                return jsonify({'status': 'error', 'message': f'Government warnings check failed', 'details': f'Active health alerts for {warning_summary}. Review restrictions before dispensing.'})
        else:
            return jsonify({'status': 'success', 'message': 'No active government warnings'})
    
    elif step == 8:  # Conflict Resolution
